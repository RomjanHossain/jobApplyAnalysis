...
class _ViewTheDatabaseState extends State<ViewTheDatabase> {
  ...
  Widget build(BuildContext context) {
    ...
    // print("Hight is ${size.height} and width is ${size.width}");
    return m.Material(
      child: ListView(
        children: [
          Text(
          ...
          m.DataTable(
              columns: const [
                ...
                m.DataColumn(label: Text('AppliedDate')),
              ...
              rows: jobs.map((job) {
                return m.DataRow(
                            ...
                            content: Center(
                              child: SingleChildScrollView(
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      "Position: ${job.$2.position}",
                                    ),
                                    Text(
                                      "Job Post: ${job.$2.jobPost}",
                                    ),
                                    Text(
                                      "Salary: ${job.$2.salary}",
                                    ),
                                    ...
                                    Text(
                                      "Company Name: ${job.$2.companyName}",
                                    ),
                                    Text(
                                      "Address: ${job.$2.address}",
                                    ),
                                    Text(
                                      "Website: ${job.$2.website}",
                                    ),
                                    Text(
                                      "Business: ${job.$2.business}",
                                    ),
                                    Text(
                  ...
                  cells: [
                    m.DataCell(
                      Text(
                        job.$2.position.fsubstring(),
                      ),
                    ),
                    m.DataCell(
                      Text(job.$2.companyName.fsubstring()),
                    ),
                    m.DataCell(
                      Text(job.$2.salary.fsubstring()),
                    ),
                    m.DataCell(
                      Text(job.$2.address.fsubstring()),
                    ),
                    m.DataCell(
                      Text(DateFormat.yMd().format(job.$2.appliedDate)),
                    ),
                    m.DataCell(
                      DropDownButton(
                        title: Text(
                            job.$2.status?.split('.').last.fcapitalize() ??
                                'Applied'),
                        items: [
                          if (job.$2.status != JobStatus.applied.toString())
                            MenuFlyoutItem(
                              ...
                              onPressed: () {
                                // update the status
                                updateStatus(
                                    JobStatus.applied.toString(), job.$1);
                              },
                            ),
                          if (job.$2.status !=
                              JobStatus.messageOrCalled.toString())
                            MenuFlyoutItem(
                              text: const Text('Messaged/Called'),
                              onPressed: () {
                                // update the status
                                updateStatus(
                                    JobStatus.messageOrCalled.toString(),
                          ...
                          if (job.$2.status != JobStatus.interview.toString())
                            MenuFlyoutItem(
                              ...
                              onPressed: () {
                                // update the status
                                updateStatus(
                                    JobStatus.interview.toString(), job.$1);
                              },
                            ),
                          if (job.$2.status != JobStatus.offered.toString())
                            MenuFlyoutItem(
                              ...
                              onPressed: () {
                                // update the status
                                updateStatus(
                                    JobStatus.offered.toString(), job.$1);
                              },
                            ),
                          if (job.$2.status != JobStatus.rejected.toString())
                            MenuFlyoutItem(
                              ...
                              onPressed: () {
                                // update the status
                                updateStatus(
                                    JobStatus.rejected.toString(), job.$1);
                              },
                            ),
                        ],
                      ),
